...
FLAMEGPU_AGENT_FUNCTION(outputdata, MsgNone, MsgSpatial3D) {
    ...
    // Set bindex for sorting
    {
        const float radius = FLAMEGPU->environment.getProperty<float>("INTERACTION_RADIUS");
        const float envMin[3] = {FLAMEGPU->environment.getProperty<float>("MIN_POSITION"), FLAMEGPU->environment.getProperty<float>("MIN_POSITION"), FLAMEGPU->environment.getProperty<float>("MIN_POSITION")};
        const float envMax[3] = {FLAMEGPU->environment.getProperty<float>("MAX_POSITION"), FLAMEGPU->environment.getProperty<float>("MAX_POSITION"), FLAMEGPU->environment.getProperty<float>("MAX_POSITION")};
        const float envWidth[3] = {(envMax[0]-envMin[0]), (envMax[1]-envMin[1]), (envMax[2]-envMin[2])};
        const unsigned int gridDim[3] = {static_cast<unsigned int>(ceilf(envWidth[0] / radius)), static_cast<unsigned int>(ceilf(envWidth[1] / radius)), static_cast<unsigned int>(ceilf(envWidth[2] / radius))};
        // Compute hash (effectivley an index for to a bin within the partitioning grid in this case)
        int gridPos[3] = {
            static_cast<int>(floorf(((FLAMEGPU->getVariable<float>("x")-envMin[0]) / envWidth[0])*gridDim[0])),
            static_cast<int>(floorf(((FLAMEGPU->getVariable<float>("y")-envMin[1]) / envWidth[1])*gridDim[1])),
            static_cast<int>(floorf(((FLAMEGPU->getVariable<float>("z")-envMin[2]) / envWidth[2])*gridDim[2]))
        };
        unsigned int bindex = (unsigned int)(
            (gridPos[2] * gridDim[0] * gridDim[1]) +   // z
            (gridPos[1] * gridDim[0]) +                    // y
            gridPos[0]);                                      // x
        FLAMEGPU->setVariable<unsigned int>("bindex", bindex);
      
    }
    return ALIVE;
}
)###";
FLAMEGPU_HOST_FUNCTION(sort_agents) {
    FLAMEGPU->agent("Boid").sort<unsigned int>("bindex", HostAgentAPI::Asc);
}
/**
...
int main(int argc, const char ** argv) {
    ...
    {   // Boid agent
        ...
        agent.newVariable<unsigned int>("bindex");
    ...
    {   // Layer #2
        LayerDescription &layer = model.newLayer();
        layer.addHostFunction(sort_agents);
    }
    {   // Layer #3
        LayerDescription &layer = model.newLayer();
